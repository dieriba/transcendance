...
export class Player {
  ...
  public movePaddle(): void {
    if (this.moveLeft) this.position.y -= this.velocity.y;
    if (this.moveRight) this.position.y += this.velocity.y;
    if (this.position.y <= 0) {
      this.position.y = 0;
    }
    if (this.position.y + this.dimension.height >= this.canvas.height) {
      this.position.y = this.canvas.height - this.dimension.height;
    }
  }
